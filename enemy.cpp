...
void GreenGhost::Ghost_Move(Grid& grid, Player& player, std::promise<void>&& prms){
	// green ghost uses random moves
	RandomAlgo(grid);
	prms.set_value();
}

void GreenGhost::RandomAlgo(Grid& grid){
	// delta for all possible moves on the grid
	int delta[4][2] = {{-1, 0}, {0, -1}, {1, 0}, {0, 1}};
	std::vector<std::vector<int>> moves;
	for(int i = 0; i < 4; i++){
	...
	if(moves.size() == 0){ 
		std::cerr << "Error: Green Enemy has no moves left!\n";
		return;
	}
	// select one of the valid moves at random
	// the pseudo random generator is seeded at the start of the game 
	int random_index = rand() % (moves.size());
	// record prev position
	prev_x = x;
	prev_y = y;
...
bool GreenGhost::random_valid_cell(int x, int y, Grid& grid){
	// acquire grid mutex
	std::lock_guard<std::mutex> lck(grid.mtx);
	...
	// prevent move to the previous cell
	if((x == prev_x) && (y == prev_y)){ return false; }

	// prevent move into the tunnel leading to the portal
	if((x == 3) && (y == 10)){ return false; }
	if((x == 15) && (y == 10)){ return false; }

	// else it is a valid cell to move
	return true;	
}